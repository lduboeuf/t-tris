...
Board.prototype.insertAt = function(column, row, particle){
    this.container[this.index(item.column, item.row)] = particle;

}

Board.prototype.moveFigure = function(moves){

    //mark array positions empty
    var i, move;
    for ( i=0; i < moves.length; i++){
        move = moves[i]
        this.container[move.from] = null;
    }
...
Board.prototype.containsParticle = function(lineNumber){
    for(var i=0; i< this.maxColumn; i++){
        if(this.container[this.index(i,lineNumber)] != null){
            return true;
        }
    }
    return false;
